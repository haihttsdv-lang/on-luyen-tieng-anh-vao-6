...
- `src/modules/curriculum/` — module **Lộ trình học** (bổ sung ngoài URD, xem [ADR 0004](./docs/adr/0004-lo-trinh-hoc-theo-giai-doan.md), [ADR 0005](./docs/adr/0005-dong-bo-nhieu-thiet-bi.md), [ADR 0007](./docs/adr/0007-trien-khai-ra-soat-lo-trinh-hoc.md)): **67 buổi** (3 buổi/tuần, 60 phút/buổi) chia 4 giai đoạn, cá nhân hóa theo điểm kiểm tra đầu vào (`CurriculumTier`), tự đẩy lịch theo tiến độ hoàn thành thực tế (`schedule.ts`), có kiểm tra tuần/tháng tự sinh (`periodicTests.ts`).
  - `CurriculumPage.tsx` — danh sách buổi học nhóm theo giai đoạn, kèm `JourneyMap` (bản đồ hành trình dạng dải chấm, bấm để mở giai đoạn + cuộn tới buổi).
  - `SessionRunnerPage.tsx` (route `/lo-trinh-hoc/:sessionId/hoc`) — **"Vào học"**: chạy tuần tự từng khối của buổi, có đồng hồ đếm ngược và ghi nhận hoàn thành từng khối (`ProgressStore.setSessionBlockProgress`), nút hành động suy từ `blockActions.ts` (`getBlockAction`). Cuối buổi, `sessionCompletion.ts` gợi ý mức tự đánh giá dựa trên dữ liệu luyện tập thật trong buổi (số câu đã làm, quiz đã "Đã nắm" chưa, tiến độ từ vựng) thay vì 3 nút cảm tính thuần túy — chặn tự chọn "Xuất sắc" khi dữ liệu quá thấp.
  - `WarmupWidget.tsx` — khối "Khởi động" đầu mỗi buổi hiện từ nghe được ngay (thẻ từ vựng có emoji cho buổi ngữ pháp; từ ví dụ thật từ ngân hàng câu hỏi cho buổi Ngữ âm).
  - **8 buổi "Học kỹ năng"** (`skillId` thay cho `topicIds`, xen giữa các buổi ngữ pháp): 2 buổi Ngữ âm, 3 buổi Đọc hiểu, 1 buổi Tìm lỗi sai, 1 buổi Viết lại câu, 1 buổi Viết đoạn văn — dạy phương pháp làm dạng bài trước khi luyện, phủ đủ các dạng bài mà trước đây lộ trình bỏ trắng hoàn toàn.
  - `weakTopics.ts` — bắt chủ điểm đang yếu nhất từ dữ liệu luyện tập thật, gắn động vào buổi luyện chuyên sâu/ôn thi tổng lực (vốn không có chủ điểm cố định).
  - `rewards.ts` — số xu thưởng/phạt theo kết quả tự đánh giá mỗi buổi (`applySessionOutcome`, dùng chung giữa `CurriculumPage` và `SessionRunnerPage`).
